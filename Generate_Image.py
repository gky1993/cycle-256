...
def Generate_Image(images, pose_code, orinpose_code, Nz, G_model, G_cycle, args):
    """
...
#            tmp_ = pose_code[start:end]
#            tmp=orinpose_code[start:end]
#            tmptensor = torch.LongTensor(tmp)
#            tmptensor_ = torch.LongTensor(tmp)
#            batch_pose_code = one_hot(tmptensor_,2)
#            batch_orin_pose_code = one_hot(tmptensor, 2)

            batch_orin_pose_code = torch.FloatTensor(orinpose_code[start:end])
            ...
            if args.cuda:
                ...
                if args.cycle:
                    batch_orin_pose_code = batch_orin_pose_code.cuda()

            batch_image, fixed_noise, batch_pose_code = \
                Variable(batch_image), Variable(fixed_noise), Variable(batch_pose_code)
            if args.cycle:
                batch_orin_pose_code = Variable(batch_orin_pose_code)

            # Generatorでイメージ生成
            generated = G_model(batch_image, batch_pose_code, fixed_noise)
            generated_cycle = G_cycle(batch_image, batch_orin_pose_code, fixed_noise)
            ...
            for j in range(minibatch_size):
                ...
                if args.cycle:
                    save_generated_cycle_image = generated_cycle[j].cpu().data.numpy().transpose(1, 2, 0)
                    save_generated_cycle_image = np.squeeze(save_generated_cycle_image)
                    save_generated_cycle_image = (save_generated_cycle_image + 1) / 2.0 * 255.
                    save_generated_cycle_image = save_generated_cycle_image[:, :, [2, 1, 0]]  # convert from BGR to RGB
                    save_dir = '{}_generated_cycle'.format(args.snapshot)
                    filename = os.path.join(save_dir, '{}.jpg'.format(str(image_number)))
                    if not os.path.isdir(save_dir): os.makedirs(save_dir)
                    print('saving {}'.format(filename))
                    misc.imsave(filename, save_generated_cycle_image.astype(np.uint8))
