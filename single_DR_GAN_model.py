...
class Discriminator256(nn.Module):
    def __init__(self, Nd, Np, channel_num):
        ...
        convLayers = [
            ...
            nn.BatchNorm2d(320),
            nn.ELU(),
            nn.ZeroPad2d((0, 1, 0, 1)),  # Bx320x16x16 -> Bx320x17x17
            nn.Conv2d(320, 320, 3, 2, 0, bias=False),  # Bx320x17x17 -> Bx320x8x8
            nn.BatchNorm2d(320),
            nn.ELU(),
            nn.Conv2d(320, 192, 3, 1, 1, bias=False),  # Bx320x8x8-> Bx192x8x8
            nn.BatchNorm2d(192),
            nn.ELU(),
            nn.Conv2d(192, 384, 3, 1, 1, bias=False),  # Bx192x8x8 -> Bx384x8x8
            nn.BatchNorm2d(384),
            nn.ELU(),
            nn.AvgPool2d(8, stride=1),  # Bx384x8x8 -> Bx384x1x1
        ]
...
class Generator256(nn.Module):
    def __init__(self, Np, Nz, channel_num):
        ...
        G_enc_convLayers = [
            nn.Conv2d(channel_num, 32, 3, 1, 1, bias=False), # Bx3x256x256 -> Bx32x256x256
            nn.BatchNorm2d(32),
            nn.ELU(),
            nn.Conv2d(32, 64, 3, 1, 1, bias=False), # Bx32x256x256 -> Bx64x256x256
            nn.BatchNorm2d(64),
            nn.ELU(),
            nn.ZeroPad2d((0, 1, 0, 1)),                      # Bx64x256x256 -> Bx64x257x257
            nn.Conv2d(64, 64, 3, 2, 0, bias=False), # Bx64x257x257 -> Bx64x128x128
            nn.BatchNorm2d(64),
            nn.ELU(),
            nn.Conv2d(64, 64, 3, 1, 1, bias=False), # Bx64x128x128 -> Bx64x128x128
            nn.BatchNorm2d(64),
            nn.ELU(),
            nn.Conv2d(64, 128, 3, 1, 1, bias=False), # Bx64x128x128 -> Bx128x128x128
            nn.BatchNorm2d(128),
            nn.ELU(),
            nn.ZeroPad2d((0, 1, 0, 1)),                      # Bx128x128x128 -> Bx128x129x129
            nn.Conv2d(128, 128, 3, 2, 0, bias=False), #  Bx128x129x129 -> Bx128x64x64
            nn.BatchNorm2d(128),
            nn.ELU(),
            nn.Conv2d(128, 96, 3, 1, 1, bias=False), #  Bx128x64x64 -> Bx96x64x64
            nn.BatchNorm2d(96),
            nn.ELU(),
            nn.Conv2d(96, 192, 3, 1, 1, bias=False), #  Bx96x64x64 -> Bx192x64x64
            nn.BatchNorm2d(192),
            nn.ELU(),
            nn.ZeroPad2d((0, 1, 0, 1)),                      # Bx192x64x64 -> Bx192x65x65
            nn.Conv2d(192, 192, 3, 2, 0, bias=False), # Bx192x65x65 -> Bx192x32x32
            nn.BatchNorm2d(192),
            nn.ELU(),
            nn.Conv2d(192, 128, 3, 1, 1, bias=False), # Bx192x32x32 -> Bx128x32x32
            nn.BatchNorm2d(128),
            nn.ELU(),
            ...
            nn.Conv2d(256, 256, 3, 2, 0, bias=False),  # Bx256x33x33 -> Bx256x16x16
            nn.BatchNorm2d(256),
            nn.ELU(),
            nn.Conv2d(256, 160, 3, 1, 1, bias=False), # Bx256x16x16 -> Bx160x16x16
            nn.BatchNorm2d(160),
            nn.ELU(),
            nn.Conv2d(160, 320, 3, 1, 1, bias=False), # Bx160x16x16 -> Bx320x16x16
            nn.BatchNorm2d(320),
            nn.ELU(),
            nn.ZeroPad2d((0, 1, 0, 1)),  # Bx320x16x16 -> Bx320x17x17
            nn.Conv2d(320, 320, 3, 2, 0, bias=False),  # Bx320x17x17 -> Bx320x8x8
        ...
        G_dec_convLayers = [
            nn.ConvTranspose2d(384, 192, 3, 1, 1, bias=False),  # Bx320x6x6 -> Bx160x6x6
            nn.BatchNorm2d(192),
            nn.ELU(),
            nn.ConvTranspose2d(192, 320, 3, 1, 1, bias=False),  # Bx160x6x6 -> Bx256x6x6
            nn.BatchNorm2d(320),
            nn.ELU(),
            nn.ConvTranspose2d(320, 320, 3, 2, 0, bias=False),  # Bx256x6x6 -> Bx256x13x13
            nn.BatchNorm2d(320),
            nn.ELU(),
            Crop([0, 1, 0, 1]),
            nn.ConvTranspose2d(320,160, 3,1,1, bias=False), # Bx320x6x6 -> Bx160x6x6
        ...
        self.G_dec_fc = nn.Linear(384+Np+Nz, 384*8*8)
    ...
    def forward(self, input, pose, noise):
        ...
        x = self.G_enc_convLayers(input) # Bxchx256x256 -> Bx384x1x1

        x = x.view(-1,384)

        self.features = x

        x = torch.cat([x, pose, noise], 1)  # Bx384 -> B x (384+Np+Nz)

        x = self.G_dec_fc(x) # B x (384+Np+Nz) -> B x (384x8x8)
        x = x.view(-1, 384, 8, 8) # B x (384x6x6) -> B x 384 x 8x8

        x = self.G_dec_convLayers(x) #  B x 384 x 8x8 -> Bxchx96x96
